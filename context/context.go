...
func (c *Context) Get(ctx gocontext.Context, name string) (*Route, error) {
...
func (c *Context) Put(ctx gocontext.Context, key string, rt *Route) error {
...
func (c *Context) Del(ctx gocontext.Context, key string) error {
	ref := c.db.Doc("routes/" + key)
...
// allowing one to ignore auto-generated IDs
// Unlike GetAll, Get, Put, or Del, since an iterator is returned, we need
// to accept context for the call so the caller can set a timeout and/or cancel
func (c *Context) List(ctx gocontext.Context, start string, ignoreGenerated bool) (*firestore.DocumentIterator, error) {
	col := c.db.Collection("routes").OrderBy(firestore.DocumentID, firestore.Asc)

	// ignore generated *before* we limit
	if ignoreGenerated {
		col = col.Where("auto_generated", "==", ignoreGenerated)
	}
...
func (c *Context) GetAll(ctx gocontext.Context) (map[string]Route, error) {
...
func (c *Context) NextID(ctx gocontext.Context) (uint32, error) {
